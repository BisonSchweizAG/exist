...
/**
 * Plugins manager. It control search procedure, activation and de-actication (including runtime).
 *
...
public class PluginsManagerImpl implements Configurable, BrokerPoolService, PluginsManager, LifeCycle {
      ...
      try {
        ...
        for (final Class<? extends Plug> pluginClazz : listServices(Plug.class)) {
          try {
            ...
            final Function<PluginsManager, Plug> ctor = (Function<PluginsManager, Plug>)LambdaMetafactory
                .metafactory(lookup, "apply", methodType(Function.class), methodHandle.type().erase(), methodHandle, methodHandle.type())
  ...
  private <S> Iterable<Class<? extends S>> listServices(final Class<S> ifc) throws Exception {
    ...
    while (e.hasMoreElements()) {
      ...
      try (final InputStream is = url.openStream(); final BufferedReader r = new BufferedReader(new InputStreamReader(is, "UTF-8"))) {
        String line;
